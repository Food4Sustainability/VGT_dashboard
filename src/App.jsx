...
export default function AgronomicPortal() {
  ...
  // --- Shapes (SVG paths/polys are defined in native 546x829 coordinates) ---
  const shapes = [
    // prior custom path
    {
      id: "custom-path-1",
  ...
  // Compute current title: selected -> that parcela; none -> Todas as Parcelas
  ...
  useEffect(() => {
    // keep focus index synced when tab changes programmatically
          ...
          <div className="min-w-[48rem] px-10 py-8 bg-gradient-to-b from-white to-chlorophyll/5">
            ...
            <div
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "ArrowRight") {
                  e.preventDefault();
                  const next = focusedTab === last ? 0 : focusedTab + 1;
                  setFocusedTab(next);
                  tabRefs.current[next]?.focus();
                } else if (e.key === "ArrowLeft") {
                  e.preventDefault();
                  const prev = focusedTab === 0 ? last : focusedTab - 1;
                  setFocusedTab(prev);
            ...
            >
              ...
              <iframe title="Hortipor (3)" width="1000" height="600" src="https://app.powerbi.com/view?r=eyJrIjoiMDMzZWEyYTctMGE3MC00ZjYyLTkxYTEtYWUyM2UxZTM4NWU0IiwidCI6IjRlNGJiMjQ3LWZjN2MtNGY3YS1iNTYzLTdiZTRmMDkzZWZkZCIsImMiOjl9&pageName=101883f99518a994bd0a" frameborder="0" allowFullScreen="true"></iframe>
            </section>

            <section
              role="tabpanel"
              id="panel-sondas"
              aria-labelledby="tab-sondas"
              hidden={tab !== "sondas"}
            >
              <h2 className="text-lg font-semibold text-chlorophyll mb-3 border-l-4 border-chlorophyll pl-3">Sondas</h2>
              <div className="bg-white shadow-soft rounded-2xl p-6 hover:shadow-lg transition-shadow">
                <p className="text-gray-700 leading-relaxed">
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque malesuada, nunc non rhoncus interdum, sem erat lacinia nisi, quis fermentum dui nisl id est.
                </p>
              </div>
            </section>
